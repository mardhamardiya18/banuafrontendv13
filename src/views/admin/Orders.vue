          ...
          <transition name="slideover-panel">
            <div v-if="showDetail" class="relative bg-white w-full max-w-2xl h-full overflow-y-auto shadow-2xl">
              ...
              <div v-else-if="detail" class="p-7 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-t border-b border-gray-100 py-4">
                  <div class="flex flex-wrap gap-2">
                    <span class="px-3 py-1.5 rounded-xl text-[11px] font-bold uppercase tracking-wider" :class="statusBadge(detail.status?.order)">Order: {{ detail.status?.order }}</span>
                    <span class="px-3 py-1.5 rounded-xl text-[11px] font-bold uppercase tracking-wider" :class="payBadge(detail.status?.payment)">Payment: {{ detail.status?.payment }}</span>
                  </div>
                  <div class="grid grid-cols-2 sm:flex gap-2 w-full sm:w-auto">
                    <button @click="openStatusModal" class="text-xs font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 px-4 py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1.5 whitespace-nowrap">
                      <i class="bx bx-edit text-sm"></i> Update Status
                    </button>
                    <button @click="handleDownloadInvoice" :disabled="downloading" class="text-xs font-semibold text-white bg-brand-maroon hover:bg-brand-maroon/90 px-4 py-2.5 rounded-xl transition-colors flex items-center justify-center gap-1.5 disabled:opacity-50 whitespace-nowrap">
